...
class Translator(object):
    ...
    def translate(self, src, tgt=None, src_dir=None, batch_size=None, attn_debug=False):
        ...
        if self.dump_beam:
            import json
            json.dump(self.translator.beam_accum, codecs.open(self.dump_beam, 'w', 'utf-8'))
    ...
    def _translate_random_sampling(self, batch, data, max_length, min_length=0,
                                   sampling_temp=1.0, keep_topk=-1, return_attention=False):
        ...
        start_token = vocab.stoi[self.fields["tgt2"].init_token]
        end_token = vocab.stoi[self.fields["tgt2"].eos_token]

        # Encoder forward.
        src, enc_states, memory_bank, src_lengths = self._run_encoder(batch, data.data_type)
        self.model.decoder2.init_state(src, memory_bank, enc_states)
        ...
        seq_so_far = torch.full([batch_size, 1], start_token, dtype=torch.long, device=mb_device)
        alive_attn = None

        for step in range(max_length):
            decoder_input = seq_so_far[:, -1].view(1, -1, 1)

            log_probs, attn = self._decode_and_generate(decoder_input, memory_bank, batch, data,
            ...
            topk_ids, topk_scores = self.sample_with_temperature(log_probs, sampling_temp, keep_topk)
        ...
        attention = (alive_attn.view(alive_attn.size(0), -1, 1, alive_attn.size(-1))
    ...
    def translate_batch(self, batch, data, attn_debug, fast=False):
        ...
        with torch.no_grad():
            if self.beam_size == 1:
                return self._translate_random_sampling(batch, data, self.max_length, min_length=self.min_length,
                                                       sampling_temp=self.random_sampling_temp,
                                                       keep_topk=self.sample_from_topk,
                                                       return_attention=attn_debug or self.replace_unk)
            if fast:
                return self._fast_translate_batch(batch, data, self.max_length, min_length=self.min_length,
                                                  n_best=self.n_best, return_attention=attn_debug or self.replace_unk)
            else:
    ...
    def _decode_and_generate(self, decoder_in, memory_bank, batch, data, memory_lengths, src_map=None,
        ...
        if self.copy_attn:
            ...
            decoder_in = decoder_in.masked_fill(decoder_in.gt(len(tgt_field.vocab) - 1), unk_idx)

        # Decoder forward, takes [tgt_len, batch, nfeats] as input
        # and [src_len, batch, hidden] as memory_bank
        # in case of inference tgt_len = 1, batch = beam times batch_size
        # in case of Gold Scoring tgt_len = actual length, batch = 1 batch
        dec_out, dec_attn = self.model.decoder2(decoder_in, memory_bank, memory_lengths=memory_lengths, step=step)
        ...
        if not self.copy_attn:
            ...
            attn = dec_attn["copy"]
            scores = self.model.generator2(dec_out.view(-1, dec_out.size(2)), attn.view(-1, attn.size(2)), src_map)
            # here we have scores [tgt_lenxbatch, vocab] or [beamxbatch, vocab]
            if batch_offset is None:
                scores = scores.view(batch.batch_size, -1, scores.size(-1))
            else:
                scores = scores.view(-1, self.beam_size, scores.size(-1))
            scores = data.collapse_copy_scores(scores, batch, tgt_field.vocab, data.src_vocabs,
                                               batch_dim=0, batch_offset=batch_offset)
    ...
    def _fast_translate_batch(self, batch, data, max_length, min_length=0, n_best=1, return_attention=False):
        ...
        self.model.decoder2.map_state(lambda state, dim: tile(state, beam_size, dim=dim))
        if isinstance(memory_bank, tuple):
        ...
        src_map = (tile(batch.src_map, beam_size, dim=1) if use_src_map else None)
        ...
        for step in range(max_length):
            ...
            if is_finished.any():
                ...
                for i in range(is_finished.size(0)):
                    ...
                    for j in finished_hyp:
                        hypotheses[b].append((topk_scores[i, j], predictions[i, j, 1:],  # Ignore start_token.
                ...
                if alive_attn is not None:
                    alive_attn = attention.index_select(1, non_finished)\
                        .view(alive_attn.size(0), -1, alive_attn.size(-1))
            ...
            self.model.decoder2.map_state(lambda state, dim: state.index_select(dim, select_indices))
    ...
    def _translate_batch(self, batch, data):
        ...
        beam = [onmt.translate.Beam(beam_size, n_best=self.n_best, cuda=self.cuda, global_scorer=self.global_scorer,
                                    pad=pad, eos=eos, bos=bos, min_length=self.min_length,
                                    stepwise_penalty=self.stepwise_penalty, block_ngram_repeat=self.block_ngram_repeat,
        ...
        results["batch"] = batch
        if "tgt2" in batch.__dict__:
            results["gold_score"] = self._score_target(batch, memory_bank, src_lengths, data, batch.src_map
        ...
        src_map = (tile(batch.src_map, beam_size, dim=1) if data.data_type == 'text' and self.copy_attn else None)
        self.model.decoder2.map_state(lambda state, dim: tile(state, beam_size, dim=dim))
        ...
        for i in range(self.max_length):
            ...
            out, beam_attn = self._decode_and_generate(inp, memory_bank, batch, data, memory_lengths=memory_lengths,
    ...
    def _score_target(self, batch, memory_bank, src_lengths, data, src_map):
        ...
        log_probs, attn = self._decode_and_generate(tgt_in, memory_bank, batch, data,
    ...
    def _report_score(self, name, score_total, words_total):
        if words_total == 0:
            ...
            msg = ("%s AVG SCORE: %.4f, %s PPL: %.4f" % (name, score_total / words_total,
    ...
    def _report_bleu(self, tgt_path):
        ...
        res = subprocess.check_output("perl %s/tools/multi-bleu.perl %s" % (base_dir, tgt_path),
                                      stdin=self.out_file, shell=True).decode("utf-8")
    ...
    def _report_rouge(self, tgt_path):
        ...
        msg = subprocess.check_output("python %s/tools/test_rouge.py -r %s -c STDIN" % (path, tgt_path),
                                      shell=True, stdin=self.out_file).decode("utf-8").strip()
        return msg
