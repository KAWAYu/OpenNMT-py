...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers, hidden_size,
                 attn_type="general", attn_func="softmax", coverage_attn=False, context_gate=None,
                 copy_attn=False, dropout=0.0, embeddings=None, reuse_copy_attn=False):
        ...
        self.attn = onmt.modules.GlobalAttention(
            hidden_size, coverage=coverage_attn, attn_type=attn_type, attn_func=attn_func)
    ...
    def init_state(self, src1, src2, memory_bank1, memory_bank2, encoder1_final, encoder2_final):
        ...
        if isinstance(encoder1_final, tuple):  # LSTM
            self.state["hidden"] = tuple([_fix_enc_hidden(enc_hid) for enc_hid in encoder1_final])
        else:  # GRU
            self.state["hidden"] = (_fix_enc_hidden(encoder1_final), )
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None):
        ...
        dec_outs, p_attn = self.attn(
            rnn_output.transpose(0, 1).contiguous(), memory_bank.transpose(0, 1), memory_lengths=memory_lengths)
        attns["std"] = p_attn

        # Calculate the context gate.
        if self.context_gate is not None:
            dec_outs = self.context_gate(
                emb.view(-1, emb.size(2)), rnn_output.view(-1, rnn_output.size(2)), dec_outs.view(-1, dec_outs.size(2)))
