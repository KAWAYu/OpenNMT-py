...
class SharedAttentionDecoder(nn.Module):
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 hidden_size, attn_type="general", attn_func="softmax",
                 coverage_attn=False, context_gate=None, shared_attn=None,
                 copy_attn=False, dropout=0.0, embeddings=None,
                 reuse_copy_attn=False):
        super(SharedAttentionDecoder, self).__init__()
        ...
        if copy_attn and not reuse_copy_attn:
            self.copy_attn = onmt.modules.GlobalAttention(hidden_size, attn_type=attn_type, attn_func=attn_func)
        if copy_attn:
            self._copy = True
