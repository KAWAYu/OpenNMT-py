...
def model_opts(parser):
    ...
    group.add('--tgt1_word_vec_size', '-tgt1_word_vec_size', type=int, default=500,
              help='Word embedding size for tgt1.')
    group.add('--tgt2_word_vec_size', '-tgt2_word_vec_size', type=int, default=500,
              help='Word embedding size for tgt2.')
    group.add('--word_vec_size', '-word_vec_size', type=int, default=-1, help='Word embedding size for src and tgt.')

    group.add('--share_decoder_embeddings', '-share_decoder_embeddings', action='store_true',
    ...
    group.add('--share_embeddings', '-share_embeddings', action='store_true',
              help="""Share the word embeddings between encoder and decoder. Need to use shared dictionary for this 
              option.""")
    ...
    group.add('--feat_merge', '-feat_merge', type=str, default='concat', choices=['concat', 'sum', 'mlp'],
    ...
    group.add('--feat_vec_size', '-feat_vec_size', type=int, default=-1,
              help="""If specified, feature embedding sizes will be set to this. Otherwise, feat_vec_exponent 
              will be used.""")
    group.add('--feat_vec_exponent', '-feat_vec_exponent', type=float, default=0.7,
              help="""If -feat_merge_size is not set, feature embedding sizes will be set to N^feat_vec_exponent 
    ...
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn', choices=['rnn', 'transformer', 'cnn'],
    ...
    group.add('--dec1_layers', '-dec1_layers', type=int, default=2, help='Number of layers in the decoder1')
    group.add('--dec2_layers', '-dec2_layers', type=int, default=2, help='Number of layers in the decoder2')
    ...
    group.add('--enc_rnn_size', '-enc_rnn_size', type=int, default=500,
              help="""Size of encoder rnn hidden states. Must be equal to dec_rnn_size except for speech-to-text.""")
    group.add('--dec1_rnn_size', '-dec1_rnn_size', type=int, default=500,
              help="""Size of decoder1 rnn hidden states. Must be equal to enc_rnn_size except for speech-to-text.""")
    group.add('--dec2_rnn_size', '-dec2_rnn_size', type=int, default=500,
              help="""Size of decoder2 rnn hidden states. Must be equal to enc_rnn_size except for speech-to-text.""")
    group.add('--audio_enc_pooling', '-audio_enc_pooling', type=str, default='1',
              help="""The amount of pooling of audio encoder, either the same amount of pooling across all layers 
              indicated by a single number, or different amounts of pooling per layer separated by comma.""")
    ...
    group.add('--input_feed', '-input_feed', type=int, default=1,
              help="""Feed the context vector at each time step as additional input 
    ...
    group.add('--rnn_type', '-rnn_type', type=str, default='LSTM', choices=['LSTM', 'GRU', 'SRU'], action=CheckSRU,
    ...
    group.add('--context_gate', '-context_gate', type=str, default=None, choices=['source', 'target', 'both'],
              help="""Type of context gate to use. Do not select for no context gate.""")

    # Attention options
    group = parser.add_argument_group('Model- Attention')
    group.add('--global_attention', '-global_attention', type=str, default='general', choices=['dot', 'general', 'mlp'],
    ...
    group.add('--self_attn_type', '-self_attn_type', type=str, default="scaled-dot",
    ...
    group.add('--generator_function', '-generator_function', default="softmax", choices=["softmax", "sparsemax"],
              help="""Which function to use for generating probabilities over the target vocabulary 
...
def preprocess_opts(parser):
    ...
    group.add('--data_type', '-data_type', default="text", help="""Type of the source input. Options are [text|img].""")

    group.add('--train_src', '-train_src', required=True, help="Path to the training source data")
    group.add('--train_tgt1', '-train_tgt1', required=True, help="Path to the training target1 data")
    group.add('--train_tgt2', '-train_tgt2', required=True, help="Path to the training target2 data")
    group.add('--valid_src', '-valid_src', required=True, help="Path to the validation source data")
    group.add('--valid_tgt1', '-valid_tgt1', required=True, help="Path to the validation target1 data")
    group.add('--valid_tgt2', '-valid_tgt2', required=True, help="Path to the validation target2 data")

    group.add('--src_dir', '-src_dir', default="", help="Source directory for image or audio files.")

    group.add('--save_data', '-save_data', required=True, help="Output file for the prepared data")

    group.add('--max_shard_size', '-max_shard_size', type=int, default=0, help="""Deprecated use shard_size instead""")

    group.add('--shard_size', '-shard_size', type=int, default=1000000,
              help="""Divide src_corpus and tgt_corpus into smaller multiple src_copus and tgt corpus files, then 
              build shards, each shard will have opt.shard_size samples except last shard. shard_size=0 means
               no segmentation shard_size>0 means segment dataset into multiple shards, 
    ...
    group.add('--tgt1_vocab', '-tgt1_vocab', default="",
              help="""Path to an existing target1 vocabulary. Format: one word per line.""")
    group.add('--tgt2_vocab', '-tgt2_vocab', default="",
              help="""Path to an existing target2 vocabulary. Format: one word per line.""")
    group.add('--features_vocabs_prefix', '-features_vocabs_prefix', type=str, default='',
              help="Path prefix to existing features vocabularies")
    group.add('--src_vocab_size', '-src_vocab_size', type=int, default=50000, help="Size of the source vocabulary")
    group.add('--tgt1_vocab_size', '-tgt1_vocab_size', type=int, default=50000, help="Size of the target1 vocabulary")
    group.add('--tgt2_vocab_size', '-tgt2_vocab_size', type=int, default=50000, help="Size of the target2 vocabulary")

    group.add('--src_words_min_frequency', '-src_words_min_frequency', type=int, default=0)
    group.add('--tgt1_words_min_frequency', '-tgt1_words_min_frequency', type=int, default=0)
    group.add('--tgt2_words_min_frequency', '-tgt2_words_min_frequency', type=int, default=0)
    ...
    group.add('--src_seq_length_trunc', '-src_seq_length_trunc', type=int, default=None,
              help="Truncate source sequence length.")
    group.add('--tgt1_seq_length', '-tgt1_seq_length', type=int, default=50,
              help="Maximum target1 sequence length to keep.")
    group.add('--tgt1_seq_length_trunc', '-tgt1_seq_length_trunc', type=int, default=None,
              help="Truncate target1 sequence length.")
    group.add('--tgt2_seq_length', '-tgt2_seq_length', type=int, default=50,
              help="Maximum target2 sequence length to keep.")
    group.add('--tgt2_seq_length_trunc', '-tgt2_seq_length_trunc', type=int, default=None,
              help="Truncate target2 sequence length.")
    ...
    group.add('--log_file_level', '-log_file_level', type=str, action=StoreLoggingLevelAction,
              choices=StoreLoggingLevelAction.CHOICES, default="0")
...
def train_opts(parser):
    ...
    group.add('--data', '-data', required=True,
              help="""Path prefix to the ".train.pt" and ".valid.pt" file path from preprocess.py""")

    group.add('--save_model', '-save_model', default='model',
              help="""Model filename (the model will be saved as <save_model>_N.pt where N is the number of steps""")

    group.add('--save_checkpoint_steps', '-save_checkpoint_steps', type=int, default=5000,
    ...
    group.add('--param_init_glorot', '-param_init_glorot', action='store_true',
              help="""Init parameters with xavier_uniform. Required for transfomer.""")
    ...
    group.add('--reset_optim', '-reset_optim', default='none', choices=['none', 'all', 'states', 'keep_states'],
    ...
    group.add('--pre_word_vecs_dec', '-pre_word_vecs_dec',
              ...
              See README for specific formatting instructions.""")
    # Fixed word vectors
    group.add('--fix_word_vecs_enc', '-fix_word_vecs_enc', action='store_true',
              help="Fix word embeddings on the encoder side.")
    group.add('--fix_word_vecs_dec', '-fix_word_vecs_dec', action='store_true',
    ...
    group.add('--batch_type', '-batch_type', default='sents', choices=["sents", "tokens"],
              help="""Batch grouping for batch_size. Standard is sents. Tokens will do dynamic batching""")
    group.add('--normalization', '-normalization', default='sents', choices=["sents", "tokens"],
              help='Normalization method of the gradient.')
    group.add('--accum_count', '-accum_count', type=int, default=1,
              help="""Accumulate gradient this many times. Approximately equivalent to updating 
    ...
    group.add('--max_generator_batches', '-max_generator_batches', type=int, default=32,
              help="""Maximum batches of words in a sequence to run the generator on in parallel. 
    ...
    group.add('--optim', '-optim', default='sgd',
              choices=['sgd', 'adagrad', 'adadelta', 'adam', 'sparseadam', 'adafactor'],
              help="""Optimization method.""")
    group.add('--adagrad_accumulator_init', '-adagrad_accumulator_init', type=float, default=0,
              help="""Initializes the accumulator values in adagrad. Mirrors the initial_accumulator_value option 
              in the tensorflow adagrad (use 0.1 for their default).""")
    group.add('--max_grad_norm', '-max_grad_norm', type=float, default=5,
              help="""If the norm of the gradient vector exceeds this, renormalize it to have the norm equal
               to max_grad_norm""")
    group.add('--dropout', '-dropout', type=float, default=0.3, help="Dropout probability; applied in LSTM stacks.")
    ...
    group.add('--adam_beta1', '-adam_beta1', type=float, default=0.9,
              help="""The beta1 parameter used by Adam. Almost without exception a value of 0.9 is used in 
              the literature, seemingly giving good results, so we would discourage changing this value from 
              the default without due consideration.""")
    group.add('--adam_beta2', '-adam_beta2', type=float, default=0.999,
              help="""The beta2 parameter used by Adam. Typically a value of 0.999 is recommended, as this is 
              the value suggested by the original paper describing Adam, and is also the value adopted 
              in other frameworks such as Tensorflow and Kerras, i.e. see: 
              https://www.tensorflow.org/api_docs/python/tf/train/AdamOptimizer 
              https://keras.io/optimizers/ . 
              Whereas recently the paper "Attention is All You Need" suggested a value of 0.98 for beta2,
               this parameter may not work well for normal models / default baselines.""")
    group.add('--label_smoothing', '-label_smoothing', type=float, default=0.0,
              help="""Label smoothing value epsilon. Probabilities of all non-true labels will be smoothed 
              by epsilon / (vocab_size - 1). Set to zero to turn off label smoothing. For more detailed information,
    ...
    group.add('--learning_rate_decay', '-learning_rate_decay', type=float, default=0.5,
              help="""If update_learning_rate, decay learning rate by this much 
              if steps have gone past start_decay_steps""")
    group.add('--start_decay_steps', '-start_decay_steps', type=int, default=50000,
    ...
    group.add('--tensorboard', '-tensorboard', action="store_true",
              help="""Use tensorboardX for visualization during training. Must have the library tensorboardX.""")
...
def translate_opts(parser):
    ...
    group.add('--model', '-model', dest='models', metavar='MODEL', nargs='+', type=str, default=[], required=True,
    ...
    group.add('--avg_raw_probs', '-avg_raw_probs', action='store_true',
              help="""If this is set, during ensembling scores from different models will be combined by 
              averaging their raw probabilities and then taking the log. Otherwise, the log probabilities 
              will be averaged directly. Necessary for models whose output layers can assign zero probability.""")
    ...
    group.add('--report_bleu', '-report_bleu', action='store_true',
              help="""Report bleu score after translation, call tools/multi-bleu.perl on command line""")
    group.add('--report_rouge', '-report_rouge', action='store_true',
              help="""Report rouge 1/2/3/L/SU4 score after translation call tools/test_rouge.py on command line""")
    ...
    group.add('--random_sampling_topk', '-random_sampling_topk', default=1, type=int,
              help="""Set this to -1 to do random sampling from full distribution. Set this to value k>1 to do random 
              sampling restricted to the k most likely next tokens. Set this to 1 to use argmax or for 
              doing beam search.""")
    group.add('--random_sampling_temp', '-random_sampling_temp', default=1., type=float,
    ...
    group.add('--fast', '-fast', action="store_true",
              help="""Use fast beam search (some features may not be supported!)""")
    ...
    group.add('--length_penalty', '-length_penalty', default='none', choices=['none', 'wu', 'avg'],
              help="""Length Penalty to use.""")
    group.add('--coverage_penalty', '-coverage_penalty', default='none', choices=['none', 'wu', 'summary'],
              help="""Coverage Penalty to use.""")
    group.add('--alpha', '-alpha', type=float, default=0.,
              help="""Google NMT length penalty parameter (higher = longer generation)""")
    group.add('--beta', '-beta', type=float, default=-0., help="""Coverage penalty parameter""")
    group.add('--block_ngram_repeat', '-block_ngram_repeat', type=int, default=0,
              help='Block repetition of ngrams during decoding.')
    group.add('--ignore_when_blocking', '-ignore_when_blocking', nargs='+', type=str, default=[],
    ...
    group.add('--replace_unk', '-replace_unk', action="store_true",
              help="""Replace the generated UNK tokens with the source token that had highest attention weight. 
              If phrase_table is provided, it will lookup the identified source token and give the corresponding 
              target token. If it is not provided(or the identified source token does not exist in the table) then it 
...
def add_md_help_argument(parser):
    """ md help parser """
    parser.add('--md', '-md', action=MarkdownHelpAction, help='print Markdown-formatted help text and exit.')
...
class MarkdownHelpFormatter(configargparse.HelpFormatter):
    ...
    def _format_action(self, action):
        if action.dest == "help" or action.dest == "md":
            return ""
        lines = []
        lines.append('* **-%s %s** ' % (action.dest, "[%s]" % action.default if action.default else "[]"))
        if action.help:
...
class MarkdownHelpAction(configargparse.Action):
    ...
    def __init__(self, option_strings, dest=configargparse.SUPPRESS, default=configargparse.SUPPRESS, **kwargs):
