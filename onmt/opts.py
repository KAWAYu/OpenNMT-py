...
def config_opts(parser):
    ...
    parser.add('-save_config', '--save_config', required=False, is_write_out_config_file_arg=True,
...
def model_opts(parser):
    ...
    group.add('--src1_word_vec_size', '-src1_word_vec_size', type=int, default=500,
              help='Word embedding size for src.')
    group.add('--src2_word_vec_size', '-src2_word_vec_size', type=int, default=500,
              help='Word embedding size for src.')
    group.add('--tgt_word_vec_size', '-tgt_word_vec_size', type=int, default=500,
    ...
    group.add('--feat_vec_exponent', '-feat_vec_exponent', type=float, default=0.7,
              help="""If -feat_merge_size is not set, feature embedding sizes will be set to 
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='rnn',
              ...
              help="""Type of encoder layer to use. Non-RNN layers are experimental. Options are 
              [rnn|brnn|mean|transformer|cnn].""")
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn', choices=['rnn', 'transformer', 'cnn'],
              help="""Type of decoder layer to use. Non-RNN layers are experimental. Options are 
              [rnn|transformer|cnn].""")

    group.add('--layers', '-layers', type=int, default=-1, help='Number of layers in enc/dec.')
    group.add('--enc1_layers', '-enc1_layers', type=int, default=2, help='Number of layers in the encoder')
    group.add('--enc2_layers', '-enc2_layers', type=int, default=2, help='Number of layers in the encoder')
    ...
    group.add('--enc1_rnn_size', '-enc1_rnn_size', type=int, default=500,
              help="""Size of encoder rnn hidden states. Must be equal to dec_rnn_size except for speech-to-text.""")
    group.add('--enc2_rnn_size', '-enc2_rnn_size', type=int, default=500,
              help="""Size of encoder rnn hidden states. Must be equal to dec_rnn_size except for speech-to-text.""")
    group.add('--dec_rnn_size', '-dec_rnn_size', type=int, default=500,
              help="""Size of decoder rnn hidden states. Must be equal to enc_rnn_size except for speech-to-text.""")
    ...
    group.add('--input_feed', '-input_feed', type=int, default=1,
              help="""Feed the context vector at each time step as additional input (via concatenation with the word 
...
def preprocess_opts(parser):
    ...
    group.add('--train_src1', '-train_src1', required=True, help="Path to the training source1 data")
    group.add('--train_src2', '-train_src2', required=True, help="Path to the training source2 data")
    group.add('--train_tgt', '-train_tgt', required=True, help="Path to the training target data")
    group.add('--valid_src1', '-valid_src1', required=True, help="Path to the validation source1 data")
    group.add('--valid_src2', '-valid_src2', required=True, help="Path to the validation source2 data")
    group.add('--valid_tgt', '-valid_tgt', required=True, help="Path to the validation target data")
    ...
    group.add('--shard_size', '-shard_size', type=int, default=1000000,
              ...
              shard_size>0 means segment dataset into multiple shards, each shard has shard_size samples""")

    # Dictionary options, for text corpus

    group = parser.add_argument_group('Vocab')
    group.add('--src1_vocab', '-src1_vocab', default="",
    ...
    group.add('--src1_vocab_size', '-src1_vocab_size', type=int, default=50000, help="Size of the source vocabulary")
    group.add('--src2_vocab_size', '-src2_vocab_size', type=int, default=50000, help="Size of the source vocabulary")
    group.add('--tgt_vocab_size', '-tgt_vocab_size', type=int, default=50000, help="Size of the target vocabulary")

    group.add('--src1_words_min_frequency', '-src1_words_min_frequency', type=int, default=0)
    group.add('--src2_words_min_frequency', '-src2_words_min_frequency', type=int, default=0)
    ...
    group.add('--src1_seq_length', '-src1_seq_length', type=int, default=50, help="Maximum source sequence length")
    group.add('--src2_seq_length', '-src2_seq_length', type=int, default=50, help="Maximum source sequence length")
    group.add('--src1_seq_length_trunc', '-src1_seq_length_trunc', type=int, default=None,
              help="Truncate source sequence length.")
    group.add('--src2_seq_length_trunc', '-src2_seq_length_trunc', type=int, default=None,
              help="Truncate source sequence length.")
    group.add('--tgt_seq_length', '-tgt_seq_length', type=int, default=50,
              help="Maximum target sequence length to keep.")
    group.add('--tgt_seq_length_trunc', '-tgt_seq_length_trunc', type=int, default=None,
    ...
    group.add('--image_channel_size', '-image_channel_size', type=int, default=3,
              choices=[3, 1], help="""Using grayscale image can training model faster and smaller""")
...
def train_opts(parser):
    ...
    group.add('--seed', '-seed', type=int, default=-1, help="""Random seed used for the experiments reproducibility.""")
    ...
    group.add('--pre_word_vecs_enc1', '-pre_word_vecs_enc1',
    ...
    group.add('--pre_word_vecs_dec', '-pre_word_vecs_dec',
              ...
              See README for specific formatting instructions.""")
    # Fixed word vectors
    group.add('--fix_word_vecs_enc1', '-fix_word_vecs_enc1', action='store_true',
              help="Fix word embeddings on the encoder side.")
    group.add('--fix_word_vecs_enc2', '-fix_word_vecs_enc2', action='store_true',
              help="Fix word embeddings on the encoder side.")
    ...
    group.add('--max_generator_batches', '-max_generator_batches', type=int, default=32,
              help="""Maximum batches of words in a sequence to run the generator on in parallel. Higher is faster, but
              uses more memory.""")
    ...
    group.add('--adam_beta2', '-adam_beta2', type=float, default=0.999,
              ...
              Typically a value of 0.999 is recommended, as this is the value suggested by the original paper describing 
              Adam, and is also the value adopted in other frameworks such as Tensorflow and Kerras, i.e. see: 
    ...
    group.add('--label_smoothing', '-label_smoothing', type=float, default=0.0,
              help="""Label smoothing value epsilon. Probabilities of all non-true labels will be smoothed by 
              epsilon / (vocab_size - 1). Set to zero to turn off label smoothing. For more detailed information, see:
    ...
    group.add('--learning_rate_decay', '-learning_rate_decay', type=float, default=0.5,
              help="""If update_learning_rate, decay learning rate by this much if steps have gone past 
              start_decay_steps""")
...
def translate_opts(parser):
    ...
    group.add('--avg_raw_probs', '-avg_raw_probs', action='store_true',
              help="""If this is set, during ensembling scores from different models will be combined by averaging their
              raw probabilities and then taking the log. Otherwise, the log probabilities will be averaged directly.
    ...
    group.add('--random_sampling_topk', '-random_sampling_topk', default=1, type=int,
              ...
              Set this to 1 to use argmax or for doing beam search.""")
